...
static const uint8_t CMD_ADCV[2] = {0x02, 0x60};
...
/* ===================== SPI ===================== */
static bool BMS_SPI_Transfer(uint8_t *tx, size_t tx_size, uint8_t *rx, size_t rx_size)
{
    uint32_t ms = 0u;
...
void BMS_WakeupIC(void)
{
    ...
    BMS_CS_SELECT();
    asm("nop"); asm("nop"); asm("nop"); // pequeño pulso corto
    BMS_CS_DESELECT();
...
static bool BMS_ReadRegister(const uint8_t cmd[2], uint8_t data_out[6], bool *pec_ok)
{
    uint8_t tx[12];
    uint8_t rx[12];
    uint16_t pec = PEC15_calc(2u, cmd);
...
BMS_CS_SELECT();
bool ok = BMS_SPI_Transfer(tx, 12u, rx, 12u);
BMS_CS_DESELECT();
...
if (!ok)
{
    ...
    len += sprintf(&dbg[len], "RX: ");

    for (int i = 0; i < 12; i++)
    {
        len += sprintf(&dbg[len], "%02X ", rx[i]);
    }
...
BMS_Error_t BMS_ReadAllCells(float *v, uint16_t *raw)
{
    ...
    /* ===================== GRUPO A ===================== */
    BMS_WakeupIC();
    if (!BMS_ReadRegister(CMD_RDCVA, data, &pec) || !pec)
        return BMS_ERR_RDCVA_PEC;

    for (int i = 0; i < 3; i++)
    {
        raw[i] = (uint16_t)data[2*i] | ((uint16_t)data[2*i+1] << 8);

        int16_t code = (int16_t)raw[i];
        v[i] = ((float)code * 150e-6f) + 1.5f;
    }

    /* ===================== GRUPO B ===================== */
    BMS_WakeupIC();
    if (!BMS_ReadRegister(CMD_RDCVB, data, &pec) || !pec)
        return BMS_ERR_RDCVB_PEC;

    for (int i = 0; i < 3; i++)
    {
        raw[i+3] = (uint16_t)data[2*i] | ((uint16_t)data[2*i+1] << 8);

        int16_t code = (int16_t)raw[i+3];
        v[i+3] = ((float)code * 150e-6f) + 1.5f;
    }

    /* ===================== GRUPO C ===================== */
    BMS_WakeupIC();
    if (!BMS_ReadRegister(CMD_RDCVC, data, &pec) || !pec)
        return BMS_ERR_RDCVC_PEC;

    for (int i = 0; i < 3; i++)
    {
        raw[i+6] = (uint16_t)data[2*i] | ((uint16_t)data[2*i+1] << 8);

        int16_t code = (int16_t)raw[i+6];
        v[i+6] = ((float)code * 150e-6f) + 1.5f;
    }

    /* ===================== GRUPO D ===================== */
    BMS_WakeupIC();
    if (!BMS_ReadRegister(CMD_RDCVD, data, &pec) || !pec)
        return BMS_ERR_RDCVD_PEC;

    raw[9] = (uint16_t)data[0] | ((uint16_t)data[1] << 8);

    int16_t code = (int16_t)raw[9];
    v[9] = ((float)code * 150e-6f) + 1.5f;
...
void BMS_SpiTestTask(void *pvParameters)
{
    ...
    char buffer[256];
    float v[TOTAL_CELLS] = {0.0f};
    uint16_t raw[TOTAL_CELLS] = {0u};
    ...
    BMS_WakeupIC();
    if (!BMS_WriteRegister(CMD_WRCFGA, cfga_write))
    {
        err = BMS_ERR_CFG_WRITE;
    }
    else
    {
        ...
        BMS_WakeupIC();
        if (!BMS_ReadRegister(CMD_RDCFGA, cfga_read, &pec_ok))
        {
            err = BMS_ERR_CFG_READ;
        }
        else if (!pec_ok)
        {
            err = BMS_ERR_CFG_PEC;
        }
        else if ((cfga_read[0] & 0x01u) == 0u)
        {
        ...
        else
        {
            ...
            memcpy(g_result.rx_cfga, cfga_read, sizeof(cfga_read));
            err = BMS_ERR_NONE;
        }
    }
    ...
    for (;;)
    {
        if (err == BMS_ERR_NONE)
        {
            err = BMS_ReadAllCells(v, raw);
        }

        snprintf(buffer, sizeof(buffer),
                 "err:%u ok:%u pec:%u | RAW: %u %u %u %u %u %u %u %u %u %u | "
