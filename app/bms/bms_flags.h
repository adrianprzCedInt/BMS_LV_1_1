...
#include "task.h"
    ...
    /* -------------------------------------------------------------------------- */
    /* STATUS FLAGS (mutually exclusive states)                                   */
    /* -------------------------------------------------------------------------- */

    #define BMS_STATUS_IDLE        (1U << 0)
    #define BMS_STATUS_DISCHARGING (1U << 1)
    #define BMS_STATUS_CHARGING    (1U << 2)
    #define BMS_STATUS_BALANCING   (1U << 3)
    #define BMS_STATUS_ERROR       (1U << 4)

    /* -------------------------------------------------------------------------- */
    /* GENERIC FLAG OPERATIONS (THREAD SAFE)                                      */
    /* -------------------------------------------------------------------------- */
    ...
    do { \
        BMS_CLEAR_STATUS(); \
        BMS_SET_FLAG(bms_status_flags, BMS_STATUS_IDLE); \
    } while(0)

    #define BMS_SET_DISCHARGING() \
    do { \
        BMS_CLEAR_STATUS(); \
        BMS_SET_FLAG(bms_status_flags, BMS_STATUS_DISCHARGING); \
    } while(0)

    #define BMS_SET_CHARGING() \
    do { \
        BMS_CLEAR_STATUS(); \
        BMS_SET_FLAG(bms_status_flags, BMS_STATUS_CHARGING); \
    } while(0)

    #define BMS_SET_BALANCING() \
    do { \
        BMS_CLEAR_STATUS(); \
        BMS_SET_FLAG(bms_status_flags, BMS_STATUS_BALANCING); \
    } while(0)

    #define BMS_SET_ERROR() \
    do { \
        BMS_CLEAR_STATUS(); \
        BMS_SET_FLAG(bms_status_flags, BMS_STATUS_ERROR); \
    } while(0)
